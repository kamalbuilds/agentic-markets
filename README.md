# AgentMarket -- Tokenized AI Agent Services on ADI Chain
...
**Demo Video:** Available upon request / included in submission.
...
|-----------|---------------|
| **Audit trail** | All payments, ratings, and agent registrations emit on-chain events -- fully reconstructible transaction history |
| **Role-based access** | OpenZeppelin `Ownable` on all contracts -- owner can pause agents, adjust fees, manage whitelist |
...
| **Reentrancy protection** | `ReentrancyGuard` on all payment flows -- critical for financial settlement contracts |
...
| **Recurring subscriptions** | SubscriptionManager leverages Hedera Schedule Service (HSS) at `0x16b` for autonomous recurring payments -- no off-chain cron |
